...
# changed : 2022-01-07 15:45:26 (Marcel Arpogaus)
# DESCRIPTION #################################################################
...
def plot_patches(
    ...
    y_off = np.where(len(meta_columns), y_vdim, 0)
    x_off = np.where(len(meta_columns), x_vdim, 0)

    x_columns = sorted(set(history_columns + meta_columns))
    ...
    for n in range(N):
        ...
        sgs = gs[0, n].subgridspec(nrows=x_ch, ncols=1, hspace=0.1, wspace=0.1)
        ...
        for col in x_columns:
            ...
            if n == 0:
                ax.set_ylabel(y_label_map.get("x", {}).get(col, col), **y_label_kw)
            if c == 0:
            ...
            if n > 0 and xy_ch_connect is not None and col == xy_ch_connect[1][0]:
                patch = matplotlib.patches.ConnectionPatch(
                    ...
                    xyB=(
                        0.0,
                        ((x_vdim + y_off) - xy_ch_connect[1][1] - 0.5)
                        / (x_vdim + y_off),
                    ),
                ...
                ax.add_artist(patch)
            if n == (N - 1) and col in history_columns:
                ax.annotate(
                    "History",
            ...
            if n == (N - 1) and col in meta_columns:
                ax.annotate(
                    "Horizon",
        ...
        for col in y_columns:
            ...
            if n == 0:
                ax.set_ylabel(y_label_map.get("y", {}).get(col, col), **y_label_kw)
            if c == 0:
...
def plot_forecast(
    ...
    columns = sorted(set(historic_columns + horizon_columns + prediction_columns))
    ...
    ax[1].plot(t_hori, pred_mu, label="mean of prediction", c="black", linewidth=2)
    ...
    for sd in sd:
        ...
        ax[1].fill(
            np.concatenate([t_hori, t_hori[::-1]]),
            np.concatenate([pred_sd_p, pred_sd_m[::-1]]),
            alpha=1 / (2 * sd),
            fc="lightskyblue",
        )
